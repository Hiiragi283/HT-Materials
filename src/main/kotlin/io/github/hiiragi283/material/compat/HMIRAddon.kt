...
object HMIRAddon : HTMaterialsAddon {
    ...
    override fun modifyMaterialRegistry(builder: HTMaterialRegistry.Builder) {
        builder.setSimple(
            NIKOLITE,
            HTMaterialComposition.molecular { color = averageColor(HTColor.DARK_BLUE, HTColor.DARK_GREEN) },
        )
    }
    ...
    override fun modifyPartManager(builder: HTPartManager.Builder) {
        listOf(
            ...
            // Chunks
            builder.add(it, CHUNK, Registries.ITEM.get(CHUNK.getId(it, modId)))
            // Purified Ores
            builder.add(it, PURIFIED_ORE, Registries.ITEM.get(PURIFIED_ORE.getId(it, modId)))
        }
        builder.add(HTMaterialKeys.WOOD, HTShapes.DUST, Registries.ITEM.get(id("sawdust")))
