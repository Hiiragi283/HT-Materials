...
class MaterialDictionaryScreenHandler(
    ...
    companion object {
        @JvmField
        val TYPE: ScreenHandlerType<MaterialDictionaryScreenHandler> = Registry.register(
            Registries.SCREEN_HANDLER,
            HTMaterialsAPI.id("material_dictionary"),
            ScreenHandlerType(::MaterialDictionaryScreenHandler, FeatureSet.empty()),
        )
    }
    ...
    override fun quickMove(player: PlayerEntity, slot: Int): ItemStack {
        ...
        val slot1: Slot = slots[slot]
        if (slot1.hasStack()) {
            val itemStack2: ItemStack = slot1.stack
            ...
            when (slot) {
                1 -> {
                    ...
                    slot1.onQuickTransfer(itemStack2, itemStack)
                }
                0 -> {
                    ...
                    } else if (slot in (2 until 29)) {
                        if (!insertItem(itemStack2, 29, 38, false)) return ItemStack.EMPTY
                    } else if (slot in (29 until 38) && !insertItem(itemStack2, 2, 29, false)) {
                        return ItemStack.EMPTY
                    }
                }
            }
            ...
            if (itemStack2.isEmpty) slot1.stack = ItemStack.EMPTY
            slot1.markDirty()
    ...
    override fun onClosed(player: PlayerEntity) {
        ...
        context.run { _, _ -> dropInventory(player, input) }
    }
}
