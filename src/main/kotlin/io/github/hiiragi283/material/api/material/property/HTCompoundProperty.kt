...
class HTCompoundProperty(map: Map<HTMaterialKey, Int>) : HTComponentProperty<HTCompoundProperty>,
    ...
    override fun asColor(): Color =
        ColorConvertible.average(this.mapKeys { (key: HTMaterialKey, _) -> HTMaterial.getMaterial(key).info.color })

    override fun asFormula(): String =
        FormulaConvertible.format(this.mapKeys { (key: HTMaterialKey, _) -> HTMaterial.getMaterial(key).info.formula })

    override fun asMolarMass(): Double =
        MolarMassConvertible.calculate(this.mapKeys { (key: HTMaterialKey, _) -> HTMaterial.getMaterial(key).info.molarMass })
