...
abstract class HTMaterialsCore {
    ...
    fun createMaterialMap(): Map<HTMaterialKey, HTMaterial> {
        ...
        val contentMapMap: HashDefaultedMap<HTMaterialKey, HTMaterialContentMap> = HashDefaultedMap.create(::HTMaterialContentMap)
        addons.forEach { it.modifyMaterialContent(contentMapMap) }
        ...
        val materialMap: Map<HTMaterialKey, HTMaterial> = buildMap {
            materialKeySet.forEach { key: HTMaterialKey ->
                ...
                val contentMap: HTMaterialContentMap = contentMapMap.getOrCreate(key)
    ...
    protected fun <T : Any> forEachContent(clazz: Class<T>, consumer: BiConsumer<HTMaterialContent<T>, HTMaterialKey>) {
        for (material: HTMaterial in HTMaterialsAPI.INSTANCE.materialRegistry().getValues()) {
            for (content: HTMaterialContent<T> in material.getContents(clazz)) {
